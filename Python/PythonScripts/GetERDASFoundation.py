...
__PRODUCTNAME__         = 'Foundation'
__PRODUCTVERSION__      = '15.0'
__MFT__                 = 'ERDAS Foundation'
__MLABEL__              = 'Foundation 2015'
...
__RECEPIENTS__          = 'D-IN-HGDImagineDev@india.ingr.com;D-IN-HGDImagineQA@india.ingr.com'

__KEEP_LAST_N_BUILDS__  = 9

# If __ENABLE_DEBUG__ is set to True, RunSystemCommand returns exitCode 0 without executing the command.
...
def main():
    ...
    # Here opts.filePath is directory path of foundation.
    ...
    PrintInfo('TourceDirectory = [%s]' % targetDirectory)
    # ------------------------------------------------------------------------------
    # Get file using NcFTPGet
    # ------------------------------------------------------------------------------
    while True:
        #exitCode = GetDirectory(opts.username, opts.password, ftpPath, targetDirectory)
        exitCode = CopyDirectoryUsingRobocopy(sourceDirectory, targetDirectory)
        PrintInfo('ExitCode = %d' % exitCode)
        if exitCode == 0: break
    ...
    PrintInfo('Copying Foundation %s -> %s' % (fileName, targetShareDir))
    # shutil.copy(fileName, targetShareDir)
    tSrcDir = targetDirectory
    tDstDir = targetShareDir
    # CopyDirectoryUsingRobocopy(targetDirectory, targetShareDir)
    CopyDirectoryUsingRobocopy(tSrcDir, tDstDir)
